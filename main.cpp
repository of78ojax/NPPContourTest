...
int main() {
	...
	while (contour.empty())
	{
		for (int i = 1; i < h_contourOffset.size() - 1; ++i)
		{
			...
			for (int i = 0; i < nMaxNumContourPoints; ++i)
			{
	...
	interImage32f.convertTo(interImage, CV_8UC1);
...
//        }
//
//        fopen_s(&bmpFile,ContoursOutputFile0.c_str(), "wb");
//
//        if (bmpFile == NULL)
//            return -1;
//        nSize = 0;
...
//        auto offset = pContoursPixelStartingOffsetHost[nCompressedLabelCount];
//
//        nppStatus = nppiCompressedMarkerLabelsUFGetGeometryListsSize_C1R(offset,
//            &nGeometryListSize);
//        auto geometryListLength = nGeometryListSize / sizeof(NppiContourPixelGeometryInfo);
//       std::vector<NppiContourPixelGeometryInfo> pContoursPixelGeometryListsHost (geometryListLength);
...
//
//
//        Npp32u blockBufferSize;
//        nppiCompressedMarkerLabelsUFGetContoursBlockSegmentListSize_C1R(pContoursPixelCountsListHost, oContoursTotalsInfoHost.nTotalImagePixelContourCount, nCompressedLabelCount, 1, nCompressedLabelCount, &blockBufferSize);
//        NppiContourBlockSegment* blockDev;
//        cudaMalloc(&blockDev, blockBufferSize);
//        std::vector<NppiContourBlockSegment> blockHost(blockBufferSize / sizeof(NppiContourBlockSegment));
//
//
...
//            pContoursPixelGeometryListsHost.data(),
...
//#endif
//
//        fopen_s(&bmpFile,ContoursReconstructedFile0.c_str(), "wb");
//
//        if (bmpFile == NULL)
//            return -1;
//        nSize = 0;
...
//
//
//    cv::Mat geom(oSizeROI.height, oSizeROI.width, CV_8UC1);
//    memcpy(geom.data, pContoursGeometryImageHost, oSizeROI.height* oSizeROI.width);
//
//    cv::imshow("geom", geom);
//    cv::waitKey(0);
//
